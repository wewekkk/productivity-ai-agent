...
export function FocusSession({ quest, subtask, initialMinutes, initialMode: requestedMode, onBack, onReport, onComplete, onRescueComplete }: { quest: Quest; subtask: Subtask; initialMinutes: number; initialMode?: FocusMode; onBack: () => void; onReport: (mode: FocusMode) => void; onComplete: () => void; onRescueComplete: () => void }) {
  const initialMode: FocusMode = requestedMode ?? (initialMinutes === 3 ? "quick" : "normal");
  const [mode, setMode] = useState<FocusMode>(initialMode);
  const [seconds, setSeconds] = useState(initialMinutes * 60);
  const [isRunning, setRunning] = useState(initialMode === "rescue");
  const [hasStarted, setHasStarted] = useState(initialMode === "rescue");
  ...
  return <main className="focus-shell"><header className="focus-header"><button className="text" onClick={leave}>← 返回任務</button><button className="text" onClick={leave}>✕ 離開</button></header><section className="focus-content"><p className="focus-quest">{mode === "rescue" ? "救援模式" : `${quest.title} · 第 ${currentStep} / ${quest.subtasks.length} 關`}</p><div className="focus-monster"><AsciiMonster state={mode === "rescue" ? "hurt" : "normal"} /></div><h1>{mode === "rescue" ? "現在只做這一步" : subtask.title}</h1>{mode === "rescue" && <p className="rescue-task">找出 3 個可以保留的 Experience bullet</p>}<div className="focus-timer" aria-label="剩餘時間">{format(seconds)}</div><p className="focus-state">{stateLabel}</p>
